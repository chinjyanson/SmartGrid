...
def problem_solver(tick=0, battery_storage=battery_initial):
    ...
    energy_demand_t = energy_demand["demand"]
    solar_energy_t = solar_energy

    # Energy balance constraint for each tick
    
    prob += buy[tick] + solar_energy_t + battery_storage * battery_efficiency - store[tick] - sell[tick] - use[tick] == energy_demand_t

    #scheduling TODO: add scheduling function here 
