...
def maximize_profit_mpc(initial_storage_level, max_storage_capacity, predicted_buy_prices, predicted_sell_prices, predicted_demand, time_step=5, horizon=15):
    ...
    for t in range(0, n - horizon, time_step):
        ...
        net = cp.Variable(horizon+1, nonneg=True)
        ...
        for i in range(horizon):
            constraints += [
                net[i+1] == storage_transactions[i] + energy_transactions[i]
                ...
                storage_level[i + 1] >= 0,
                storage_level[i] >= 0,
                storage_level[i] <= max_storage_capacity,
                pos_storage_transactions[i] <= storage_level[i],
                neg_storage_transactions[i] <= max_storage_capacity - storage_level[i],
